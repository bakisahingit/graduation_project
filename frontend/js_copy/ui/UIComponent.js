...
export class UIComponent {
    ...
    setupEventListeners() {
        ...
        // Dropdown ve Select listenler'ı
        this.setupDropdownListeners();
        this.setupInputModelSelects();
    }

    /**
     * Dropdown menü listener'ları (Add, Tools vb.)
     */
    setupDropdownListeners() {
        const dropdowns = [
            { btn: this.elements.welcomeAddBtn, menu: this.elements.welcomeAddDropdown },
            { btn: this.elements.chatAddBtn, menu: this.elements.chatAddDropdown },
            { btn: this.elements.welcomeToolsBtn, menu: this.elements.welcomeToolsDropdown },
            { btn: this.elements.chatToolsBtn, menu: this.elements.chatToolsDropdown }
        ];
        ...
        dropdowns.forEach(({ btn, menu }) => {
            if (btn && menu) {
                DOMUtils.on(btn, 'click', (e) => {
                    ...
                    dropdowns.forEach(d => {
                        if (d.menu !== menu) DOMUtils.removeClass(d.menu, 'open');
                    });

                    // Toggle current
                    DOMUtils.toggleClass(menu, 'open');
                });
            }
        });
            ...
            dropdowns.forEach(({ btn, menu }) => {
                if (menu && menu.classList.contains('open')) {
                    if (!menu.contains(e.target) && !btn.contains(e.target)) {
                        DOMUtils.removeClass(menu, 'open');
                    }
                }
            });
        });
    ...
    /**
     * Input wrapper içindeki model seçici listener'ları
     */
    setupInputModelSelects() {
        const selects = [
            {
                trigger: this.elements.selectTriggerWelcome,
                options: this.elements.selectOptionsWelcome,
                valueEl: this.elements.selectValueWelcome,
                native: this.elements.modelSelectWelcome
            },
            {
                trigger: this.elements.selectTriggerChat,
                options: this.elements.selectOptionsChat,
                valueEl: this.elements.selectValueChat,
                native: this.elements.modelSelectChat
            }
        ];

        selects.forEach(({ trigger, options, valueEl, native }) => {
            if (trigger && options) {
                // Trigger click - Toggle options
                DOMUtils.on(trigger, 'click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();

                    // Diğerlerini kapat
                    selects.forEach(s => {
                        if (s.options !== options) DOMUtils.removeClass(s.options, 'open');
                    });

                    // Chat modunda yukarı açılması gerekebilir
                    if (trigger.id === 'select-trigger-chat') {
                        DOMUtils.addClass(options, 'open-upward');
                    }

                    DOMUtils.toggleClass(options, 'open');
                    DOMUtils.toggleClass(trigger, 'active');
                });

                // Options click handling
                DOMUtils.on(options, 'click', (e) => {
                    const option = e.target.closest('.select-option-input');
                    if (option) {
                        e.stopPropagation();
                        ...
                        // Update Active State
                        const allOptions = options.querySelectorAll('.select-option-input');
                        allOptions.forEach(o => DOMUtils.removeClass(o, 'selected'));
                        DOMUtils.addClass(option, 'selected');

                        // Close
                        DOMUtils.removeClass(options, 'open');
                        DOMUtils.removeClass(trigger, 'active');
                    }
                });
            }
        });

        // Dışarı tıklama - Select kapat
        DOMUtils.on(document, 'click', (e) => {
            selects.forEach(({ trigger, options }) => {
    ...
    /**
     * Modal tetikleyicilerini bağla
     * @param {Object} callbacks - Modal açma fonksiyonları
     */
    bindModalTriggers(callbacks) {
        const { openSettings, openMolecule, openCompare, openAdmetSettings } = callbacks;

        // Settings
        if (this.elements.settingsBtn) {
            DOMUtils.on(this.elements.settingsBtn, 'click', openSettings);
        }

        // Molecule Draw
        if (this.elements.welcomeMoleculeDraw) {
            DOMUtils.on(this.elements.welcomeMoleculeDraw, 'click', openMolecule);
        }
        if (this.elements.chatMoleculeDraw) {
            DOMUtils.on(this.elements.chatMoleculeDraw, 'click', openMolecule);
        }

        // Compare Tool
        if (this.elements.welcomeCompareTool) {
            DOMUtils.on(this.elements.welcomeCompareTool, 'click', openCompare);
        }
        if (this.elements.chatCompareTool) {
            DOMUtils.on(this.elements.chatCompareTool, 'click', openCompare);
        }

        // ADMET Settings
        if (this.elements.welcomeAdmetSettingsBtn) {
            DOMUtils.on(this.elements.welcomeAdmetSettingsBtn, 'click', openAdmetSettings);
        }
        if (this.elements.chatAdmetSettingsBtn) {
            DOMUtils.on(this.elements.chatAdmetSettingsBtn, 'click', openAdmetSettings);
        }
    }

    /**
     * Modal kapatma butonlarını bağla
     * @param {Object} callbacks - Modal kapatma fonksiyonları
     */
    bindModalClosers(callbacks) {
        const { closeSettings, closeMolecule, closeCompare, closeAdmetSettings } = callbacks;

        // Settings Close
        if (this.elements.settingsClose) {
            DOMUtils.on(this.elements.settingsClose, 'click', closeSettings);
        }
        // Overlay click
        if (this.elements.settingsOverlay) {
            DOMUtils.on(this.elements.settingsOverlay, 'click', closeSettings);
        }

        // Molecule Close
        if (this.elements.moleculeClose) {
            DOMUtils.on(this.elements.moleculeClose, 'click', closeMolecule);
        }
        // Overlay click
        if (this.elements.moleculeOverlay) {
            DOMUtils.on(this.elements.moleculeOverlay, 'click', closeMolecule);
        }

        // Compare Close
        if (this.elements.compareClose) {
            DOMUtils.on(this.elements.compareClose, 'click', closeCompare);
        }
        // Overlay click
        if (this.elements.compareOverlay) {
            DOMUtils.on(this.elements.compareOverlay, 'click', closeCompare);
        }
        // Cancel button
        const compareCancel = DOMUtils.select('#compare-cancel'); // Varsayım: HTML'de varsa
        if (compareCancel) {
            DOMUtils.on(compareCancel, 'click', closeCompare);
        }

        // ADMET Settings Close
        if (this.elements.admetSettingsClose) {
            DOMUtils.on(this.elements.admetSettingsClose, 'click', closeAdmetSettings);
        }
        if (this.elements.admetSettingsCancelBtn) {
            DOMUtils.on(this.elements.admetSettingsCancelBtn, 'click', closeAdmetSettings);
        }
        // Overlay click
        if (this.elements.admetSettingsOverlay) {
            DOMUtils.on(this.elements.admetSettingsOverlay, 'click', closeAdmetSettings);
        }
    }
